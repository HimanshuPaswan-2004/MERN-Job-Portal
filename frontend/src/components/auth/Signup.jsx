                ...
                <form onSubmit={submitHandler} className='w-1/2 border border-border rounded-md p-4 my-10 shadow-lg bg-card text-card-foreground'>
                    ...
                    <div className='flex items-center justify-between my-4'>
                        ...
                        <div className="flex items-center gap-2">
                            <Label>Profile</Label>
                            <Input accept="image/*" type="file" onChange={changeFileHandler} className="cursor-pointer w-fit" />
                        </div>
                    </div>
                    {loading ? <Button className="w-full my-4" disabled>Please wait...</Button> : <Button type="submit" className="w-full my-4 bg-primary hover:bg-primary/90 text-primary-foreground">Signup</Button>}
                    <span className='text-sm text-muted-foreground'>Already have an account? <Link to="/login" className='text-primary hover:underline'>Login</Link></span>
                </form>
            </div>
        </div>
    )
